...
// Shared "finalize a verified registration" logic — writes to MongoDB +
// Google Sheets, generates the QR, and emails the pass. Called once OTP
// verification succeeds (see app/api/otp/verify/route.js). Kept separate
// from the OTP routes so it's easy to unit-test / reuse.
export async function finalizeRegistration(data) {
  const { studentType, name, email, phone, college, passingYear } = data;
  ...
  try {
    ...
    results.whatsapp = await sendWhatsappPass(registration.phone, { ticketId, qrDataUrl });
  } catch (err) {
